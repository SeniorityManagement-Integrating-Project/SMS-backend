...
from src.skill_validation_request.mappers import (
    ...
    to_skill_validation_request_with_comments,
    to_skill_validation_request_comment,
)
...
def create_comment(
    ...
    with Session(engine) as session:
        ...
        if skill_request_db.validated:
            raise HTTPException(
                status_code=400, detail="Cannot comment an already validated skill request"
            )
        ...
        new_comment = SkillValidationRequestComment(
            **request_comment.dict(), request_id=skill_validation_request_id
        )
